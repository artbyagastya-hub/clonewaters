...
function renderReleases(filter) {
  ...
  for (var idx = 0; idx < products.length; idx++) {
    ...
    if (p.img) {
      ...
      var g = grads[p.accent] || grads["g"];
      imgHtml = '<div class="placeholder-can" style="background:' + g + ';color:var(--neon)">' + p.name + "</div>";
    }
...
function addToCart(id) {
  ...
  if (p.soldOutAt > 0 && p.stock <= p.soldOutAt) {
    ...
    if (activeFilter) {
      renderReleases(activeFilter.getAttribute("data-filter"));
    }
  }
}
...
function bindAddToCart() {
  ...
  for (var i = 0; i < btns.length; i++) {
    btns[i].addEventListener("click", function() {
      var productId = parseInt(this.getAttribute("data-add"));
      addToCart(productId);
    });
  }
}
...
function goToStep(step) {
  ...
  var navBtns = document.querySelectorAll(".checkout-nav-btn");
  ...
  for (var k = 0; k < navBtns.length; k++) {
    if (navBtns[k].getAttribute("data-step") === String(step)) {
...
function renderCart() {
  ...
  for (var i = 0; i < cart.length; i++) {
    ...
    row.innerHTML = '<div class="cart-item-info">' +
      ...
      '<button class="qty-btn" data-qi="' + i + '" data-qd="-1">−</button>' +
      "<span>" + item.qty + "</span>" +
      '<button class="qty-btn" data-qi="' + i + '" data-qd="1">+</button>' +
      "</div>" +
  ...
  // Bind qty buttons
  var qtyBtns = itemsEl.querySelectorAll(".qty-btn");
  ...
  // Bind remove buttons
  var remBtns = itemsEl.querySelectorAll(".cart-item-remove");
...
function renderSummary() {
  ...
  for (var i = 0; i < cart.length; i++) {
    ...
    subtotal = subtotal + t;
    itemsHtml += '<div style="display:flex;justify-content:space-between;padding:.3rem 0;font-size:.82rem"><span>' + cart[i].name + " x " + cart[i].qty + "</span><span>" + t + "K</span></div>";
  }
...
function placeOrder() {
  ...
  for (var i = 0; i < cart.length; i++) {
    orderData.total = orderData.total + (cart[i].price * cart[i].qty);
  }
...
function showToast(msg, type) {
  ...
  t.style.cssText = "padding:.8rem 1.2rem;background:#111;border:1px solid #2a2a38;border-radius:10px;font-size:.82rem;color:#f0f0f5;box-shadow:0 10px 30px rgba(0,0,0,.4);margin-bottom:.5rem;transition:all .3s ease;max-width:320px";

  if (type === "success") {
    t.style.borderLeft = "3px solid #39ff14";
  } else if (type === "error") {
    t.style.borderLeft = "3px solid #ff3b3b";
  } else {
    t.style.borderLeft = "3px solid #00d4ff";
  }
...
for (var si = 0; si < links.length; si++) {
  links[si].addEventListener("click", function(e) {
...
// EVENT BINDINGS
// =============================================
var ageYesBtn = document.getElementById("ageYes");
if (ageYesBtn) {
  ageYesBtn.addEventListener("click", enterSite);
}

var ageNoBtn = document.getElementById("ageNo");
if (ageNoBtn) {
  ageNoBtn.addEventListener("click", leaveSite);
}
...
if (registerFormEl) {
  registerFormEl.addEventListener("submit", doRegister);
}
...
if (cartBtnEl) {
  cartBtnEl.addEventListener("click", openCheckout);
}

var closeCheckoutEl = document.getElementById("closeCheckout");
if (closeCheckoutEl) {
  closeCheckoutEl.addEventListener("click", closeCheckout);
}

var toStep2El = document.getElementById("toStep2");
if (toStep2El) {
  toStep2El.addEventListener("click", function() { goToStep(2); });
}

var toStep3El = document.getElementById("toStep3");
if (toStep3El) {
  toStep3El.addEventListener("click", function() { goToStep(3); });
}

var backToStep1El = document.getElementById("backToStep1");
if (backToStep1El) {
  backToStep1El.addEventListener("click", function() { goToStep(1); });
}

var backToStep2El = document.getElementById("backToStep2");
if (backToStep2El) {
  backToStep2El.addEventListener("click", function() { goToStep(2); });
}

var placeOrderBtnEl = document.getElementById("placeOrderBtn");
if (placeOrderBtnEl) {
  placeOrderBtnEl.addEventListener("click", placeOrder);
}

var continueShoppingEl = document.getElementById("continueShopping");
if (continueShoppingEl) {
  continueShoppingEl.addEventListener("click", closeCheckout);
}
...
for (var fi = 0; fi < filterBtns.length; fi++) {
  filterBtns[fi].addEventListener("click", function() {
    ...
    for (var fb = 0; fb < allBtns.length; fb++) {
      allBtns[fb].classList.remove("active");
    }
...
var checkoutNavBtns = document.querySelectorAll(".checkout-nav-btn");
for (var cni = 0; cni < checkoutNavBtns.length; cni++) {
  checkoutNavBtns[cni].addEventListener("click", function() {
    goToStep(parseInt(this.getAttribute("data-step")));
  });
}

var paymentOpts = document.querySelectorAll(".payment-option");
for (var pi = 0; pi < paymentOpts.length; pi++) {
  paymentOpts[pi].addEventListener("click", function() {
    var allOpts = document.querySelectorAll(".payment-option");
    for (var po = 0; po < allOpts.length; po++) {
      allOpts[po].classList.remove("active");
    }
    this.classList.add("active");
  });
}
